...
tidy_timeseries_dataset[which.max(tidy_timeseries_dataset$Sum),]$Interval
```
...
2. Devise a strategy for filling in all of the missing values in the dataset. The strategy does not need to be sophisticated. Here we have used mean for that day
```{r}
